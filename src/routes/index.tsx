...
import { getObjectAccess, listObjects } from '../features/explorer/s3.server'
import type { ExplorerResponse } from '../features/explorer/types'
import {
  extractName,
  formatDate,
  formatSize,
...
export const Route = createFileRoute('/')({
  ...
  loader: async ({ deps }) =>
    listObjects({ data: { ...deps, limit: deps.limit ?? 50 } }) as Promise<ExplorerResponse>,
...
function ExplorerPage() {
  ...
  const handleFolderSelect = (prefix: string) => {
    navigate({ search: (prev) => ({ ...prev, prefix }) })
  }
    ...
    <main className="mx-auto max-w-6xl px-6 py-10 space-y-10">
      <header className="flex flex-col gap-6 rounded-3xl border border-white/10 bg-white/5 p-8 shadow-2xl shadow-cyan-500/10 backdrop-blur">
        ...
        <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-3">
          ...
          <InfoPill
            ...
            value={`${data.objects.length} éléments`}
            icon={<FileText size={18} />}
          />
          <InfoPill
            title="Dossiers détectés"
            value={`${data.folders.length} dossiers`}
      ...
      <section className="grid gap-6 lg:grid-cols-[280px_1fr_360px]">
        ...
        <div className="space-y-4">
          <Panel title="Dossiers" description="Navigation rapide par préfixe">
